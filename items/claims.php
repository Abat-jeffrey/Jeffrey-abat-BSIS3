...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $itemCheck = db()->prepare('SELECT claim_status FROM items WHERE id = ? LIMIT 1');
    $itemCheck->execute([$itemId]);
    $item = $itemCheck->fetch();

    if ($item && $item['claim_status'] !== 'claimed') {
        $up1 = db()->prepare('UPDATE claims SET claimant_name = ?, status = "approved", approved_at = NOW(), approved_by = ? WHERE id = ? AND status = "pending"');
...
$claims = db()->query('SELECT c.*, i.item_name, i.status AS item_status, i.claim_status FROM claims c JOIN items i ON i.id = c.item_id ORDER BY c.created_at DESC')->fetchAll();
            ...
            <div class="table-responsive"><table class="table"><thead><tr><th>Item</th><th>Type</th><th>Claimant</th><th>Status</th><th>Requested</th><th>Action</th></tr></thead><tbody>
                <?php foreach ($claims as $c): ?>
                    <tr>
                        ...
                        <td>
                            <?php if ($c['status'] === 'pending' && $c['claim_status'] !== 'claimed'): ?>
